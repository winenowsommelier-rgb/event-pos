...
def fallback_enrich(product: Dict[str, str], web_context: str = "") -> Dict[str, Any]:
    ...
    if web_context:
        remark += " with scraped web context"
...
def fetch_url_text(url: str, timeout: int = 12) -> str:
    ...
    req = request.Request(url, headers={"User-Agent": "Mozilla/5.0 (WineNow-Product-Intelligence/1.0)"})
...
def pick_source_url(product: Dict[str, str]) -> str:
    for key in ["source_url", "url", "product_url", "link"]:
...
def call_gemini(product: Dict[str, str], web_context: str = "") -> Tuple[Dict[str, Any], int]:
    ...
    system_msg = (
        ...
        "product_name_standard,category,style,classification,country,region,sub_region,"
        "short_description_en,short_description_th,confidence_score,remark. "
        "Category must be one of Wine, Spirit, Beer, Sake, Accessory, Other."
    )
    prompt_payload = {
        "product_row": product,
        "scraped_web_context": web_context,
        "instructions": "Use web context as supporting evidence if available.",
    }
    ...
    usage = int(response_payload.get("usageMetadata", {}).get("totalTokenCount", 0))
    return normalize_record(json.loads(text)), usage


def process_batch(rows: List[Dict[str, str]], batch_id: int, use_web_context: bool = True) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for raw_row in rows:
        ...
        try:
            if use_web_context:
                ...
                if source_url:
                    try:
                        web_context = fetch_url_text(source_url)
                    except Exception as web_exc:  # pylint: disable=broad-except
                        error_message = f"web_scrape_warning: {web_exc}"

            try:
                enriched, token_usage = call_gemini(row, web_context)
                status = "success"
            except Exception as gemini_exc:  # pylint: disable=broad-except
                enriched = normalize_record(fallback_enrich(row, web_context))
                token_usage = 0
                status = "fallback"
                error_message = (error_message + " | " if error_message else "") + f"gemini_warning: {gemini_exc}"
...
def process_rows(rows: List[Dict[str, str]], batch_size: int, use_web_context: bool = True) -> Dict[str, Any]:
    ...
    for i in range(0, len(rows), size):
        batch_id = (i // size) + 1
        enriched, logs = process_batch(rows[i : i + size], batch_id, use_web_context=use_web_context)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        scrape_flags = form.get("use_web_context", ["1"])
        if not files:
        ...
        use_web_context = str(scrape_flags[0]).strip() in {"1", "true", "True", "on", "yes"}
        ...
        result = process_rows(rows, batch_size, use_web_context=use_web_context)
...
def run_server():
    ...
    print("Set GEMINI_API_KEY before run to enable Gemini enrichment.")
